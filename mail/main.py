...
from utils.predictor import predict_notice
...
class CheckMail:
    ...
    def save_path_in_json(self, file_path, save_in_folder, name):
        ...
        if os.path.exists(os.path.join(save_in_folder, f'{name}.json')):
            ...
            with open(os.path.join(save_in_folder, f'{name}.json'), 'w') as f:
                ...
                pdf_file_path_array.append(
                    file_path) if file_path not in pdf_file_path_array else None
    ...
    def save_file(self, source_file, destination_folder_name, root_folder_name='data'):
        ...
        new_file_path = os.path.join(
            destination_folder, source_file.split('/')[-1])
        ...
        self.save_path_in_json(
            new_file_path, save_json_file_path, destination_folder_name)
    ...
    def check_pdf(self, file_path, file):
        ...
        if is_pdf(file_path):
            ...
            # Send image to notice model if it's notice then save it.
            self.save_file(file_path, 'IMG') if predict_notice(
                file_path, 224) else None
        else:
            try:
                os.remove(file_path)
